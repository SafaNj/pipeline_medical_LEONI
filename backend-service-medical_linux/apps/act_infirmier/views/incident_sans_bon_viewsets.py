...
from apps.act_infirmier.serializers import IncidentSansBonSerializer
from apps.consultations.permissions import IsInfirmierOrMedecin


class IncidentSansBonViewSet(SiteScopedQuerysetCreateMixin, viewsets.ModelViewSet):
    queryset = IncidentSansBon.objects.select_related("collaborateur", "infirmiere")
    ...
    def by_mois(self, request):
        ...
        queryset = self.get_queryset().filter(
            date_incident__year=annee, date_incident__month=mois
        )
        serializer = self.get_serializer(queryset, many=True)
        return Response(serializer.data)
    ...
    def stats(self, request):
        ...
        if annee:
            try:
                qs = qs.filter(date_incident__year=int(annee))
            except (TypeError, ValueError):
                return Response(
        ...
        par_mois = list(
            qs.values("date_incident__month")
            .annotate(total=Count("id"))
            .order_by("date_incident__month")
        )

        return Response(
            {"total": total, "par_segment": par_segment, "par_mois": par_mois}
        )
