...
BASE = '/api/surveillance-speciale/listes-surveillance-speciale/'
BASE_L = '/api/surveillance-speciale/lignes-surveillance-speciale/'

class SSListeActionsTests(TestCase):
    def setUp(self):
    ...
    def test_pour_medecin_action(self):
        self.assertIn(auth(self.med).get(f'{BASE}pour_medecin/').status_code, [200, 400, 403])

    def test_existe_action(self):
        self.assertIn(auth(self.med).get(f'{BASE}existe/').status_code, [200, 400, 403])

    def test_medecins_travail_action(self):
        self.assertIn(auth(self.med).get(f'{BASE}medecins_travail/').status_code, [200, 403])

    def test_medecins_travail_infirmier(self):
        self.assertIn(auth(self.inf).get(f'{BASE}medecins_travail/').status_code, [200, 403])


class SSLigneActionsTests(TestCase):
    def setUp(self):
        self.site = creer_site()
        self.med = creer_user(self.site, 'medecin')
        self.inf = creer_user(self.site, 'infirmier')
    ...
    def test_create_ligne_infirmier(self):
        self.assertIn(auth(self.inf).post(BASE_L, {}, format='json').status_code, [201, 400, 403])
