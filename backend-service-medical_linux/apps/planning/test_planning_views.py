...
from apps.account.models import Site, Profile, Medecin, Infirmier


def creer_site():
    uid = uuid.uuid4().hex[:6]
    return Site.objects.create(nom=f'Site {uid}', code=f'PL_{uid}')
...
class ListePassagePlanningTests(TestCase):
    def setUp(self):
    ...
    def test_liste_passage_medecin_200(self):
        response = self.client.get('/api/planning/listes/')
        self.assertIn(response.status_code, [200, 404])

    def test_liste_passage_sans_token_401(self):
        response = APIClient().get('/api/planning/listes/')
        self.assertIn(response.status_code, [401, 403])

    def test_liste_passage_infirmier_acces(self):
        inf = creer_infirmier(self.site)
        client = APIClient()
        token = get_token(client, inf)
        client.credentials(HTTP_AUTHORIZATION=f'Bearer {token}')
        response = client.get('/api/planning/listes/')
        self.assertIn(response.status_code, [200, 403, 404])

    def test_detail_inexistant_404(self):
        response = self.client.get('/api/planning/listes/99999/')
