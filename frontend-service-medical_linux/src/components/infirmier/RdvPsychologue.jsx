// src/components/infirmier/RdvPsychologue.jsx
import { useState, useEffect } from 'react';
import {
  getRdvPsychologue,
  creerRdvPsychologue,
  modifierRdvPsychologue,
  supprimerRdvPsychologue,
...
const IcoPsycho = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/><circle cx="12" cy="12" r="3"/></svg>;
...
/* ══════════════════════════════════════════════════════════
   PANNEAU DÉTAILS
══════════════════════════════════════════════════════════ */
function DetailRdvPsychologue({ item: m, onEdit, onClose }) {
  const { user } = useAuth();
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'18px 20px' }}>
        <div style={{ marginBottom:18 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <Row label="Service" value={m.service} />
            <Row label="Position / Poste" value={m.position} />
            <Row label="Supérieur hiérarchique" value={m.superieur_hierarchique} />
...
/* ══════════════════════════════════════════════════════════
   FORMULAIRE
══════════════════════════════════════════════════════════ */
function FormulaireRdvPsychologue({ initial, onSaved, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = { collaborateur: form.collaborateur, date_rdv: form.date_rdv };
      const saved = isEdit
        ? await modifierRdvPsychologue(form.id, payload)
        : await creerRdvPsychologue(payload);
      onSaved(saved);
    } catch (err) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
      <div style={{ background:'white', borderRadius:16, width:'min(560px,96vw)', maxHeight:'90vh', display:'flex', flexDirection:'column', boxShadow:'0 20px 60px rgba(0,0,0,.22)', animation:'modalIn .2s ease' }}>
        <div style={{ padding:'18px 22px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, background:'linear-gradient(135deg,#f0f9ff,#e0f2fe)', borderRadius:'16px 16px 0 0' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:38, height:38, borderRadius:10, background:'linear-gradient(135deg,#0ea5e9,#0369a1)', display:'flex', alignItems:'center', justifyContent:'center', color:'white' }}><IcoPsycho /></div>
            <div>
              <div style={{ fontSize:15, fontWeight:800, color:'#111827' }}>{isEdit ? 'Modifier le RDV' : 'Nouveau RDV Psychologue'}</div>
              <div style={{ fontSize:11.5, color:'#6b7280' }}>Psychologue du travail</div>
            </div>
          </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 22px' }}>
          ...
          <div style={{ marginBottom:20 }}>
            <SecTitle>Date du rendez-vous</SecTitle>
          ...
          <div style={{ background:'#f0f9ff', border:'1px solid #7dd3fc', borderRadius:9, padding:'10px 14px', fontSize:12.5, color:'#075985' }}>
            ℹ️ Les informations du collaborateur (service, poste, site, supérieur hiérarchique, téléphone) sont automatiquement récupérées depuis le dossier du collaborateur.
          </div>
        </div>
...
/* ══════════════════════════════════════════════════════════
   PAGE PRINCIPALE
══════════════════════════════════════════════════════════ */
export default function RdvPsychologue() {
  ...
  const load = async () => {
    setLoading(true);
    try { setData(await getRdvPsychologue()); } catch { setData([]); }
  ...
  const handleDelete = async (id) => {
    try { await supprimerRdvPsychologue(id); setConfirmDel(null); if (selected?.id === id) setSelected(null); load(); } catch {}
  };
        ...
        <div style={{ background:'white', borderRadius:14, border:'1.5px solid #f3f4f6', display:'flex', flexDirection:'column', overflow:'hidden', boxShadow:'0 2px 12px rgba(0,0,0,.06)' }}>
          <div style={{ padding:'14px 18px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, background:'#f0f9ff' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <div style={{ width:32, height:32, borderRadius:9, background:'linear-gradient(135deg,#0ea5e9,#0369a1)', display:'flex', alignItems:'center', justifyContent:'center', color:'white' }}><IcoPsycho /></div>
              <div style={{ fontSize:14, fontWeight:800, color:'#111827' }}>RDV Psychologue du travail</div>
            </div>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                <thead>
                  <tr style={{ background:'#f9fafb' }}>
                    {['Collaborateur','Date RDV','Site','Service','Actions'].map(h => (
                ...
                <tbody>
                  {filtered.map(d => (
                    <tr key={d.id} onClick={() => setSelected(d)}
                      style={{ cursor:'pointer', transition:'background .12s', background: selected?.id === d.id ? '#f0f9ff' : 'transparent' }}
                      onMouseEnter={e => { if (selected?.id !== d.id) e.currentTarget.style.background='#f0f9ff'; }}
                      ...
                      <td style={{ padding:'10px 14px', borderBottom:'1px solid #f9fafb' }}>
                        <span style={{ fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:99, background:'#ede9fe', color:'#7c3aed', border:'1px solid #c4b5fd' }}>
                          {fmtDate(d.date_rdv)}
                        </span>
                      </td>
                      <td style={{ padding:'10px 14px', borderBottom:'1px solid #f9fafb', color:'#374151' }}>{d.site || '—'}</td>
                      <td style={{ padding:'10px 14px', borderBottom:'1px solid #f9fafb', color:'#6b7280', fontSize:12 }}>{d.service || '—'}</td>
                      <td style={{ padding:'10px 14px', borderBottom:'1px solid #f9fafb' }}>
                        <div style={{ display:'flex', gap:6 }} onClick={e => e.stopPropagation()}>
                          <button onClick={() => { setEditItem(d); setShowForm(true); }}
                            style={{ width:30, height:30, borderRadius:7, border:'1.5px solid #e5e7eb', background:'white', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'#0284c7' }} title="Modifier">
                            <IcoEdit />
                          </button>
